...
for nTweak, rCosts, calcC, contCost, adj, mon, size in zip(nodeTweak,randomCosts,calcCent,container,adjr,monitor,netSizes):
    dataSet.write("\n%d,%s,%s,%f,%d,%s,%f" % (size,rCosts,mon,nTweak,adj,calcC,contCost))
    dataSet.flush()
    results=[]
    result=0
    for j in range(runs):
    ...
    avgs.write("\n%d,%s,%s,%f,%d,%s,%f,%d" % (size,rCosts,mon,nTweak,adj,calcC,contCost,result/runs))
    avgs.flush()
dataSet.close()
avgs.close()
